...
function pageChng(rooms, diff) {
	...
	for(var room=0;room<shownRooms.length;room++) {
		$('#roomList').append('<div style="font-size: 1.8vh; background-color: #4C767B; color: #334F52; font-size: 2vh; font-weight: 900; padding: 0.8vh; margin: 0.8vh;" onclick="srv.emit(\'switchRoom\',\''+shownRooms[room].roomName+'\');">' + shownRooms[room].roomName + '<span style="float: right;">'+shownRooms[room].playerCount+'/'+shownRooms[room].maxPlayers+'</span></div>');
	}
}